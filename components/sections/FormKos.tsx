...
import { Check, Calendar, Info, Search, ShieldCheck, Clock, Handshake, Star } from "lucide-react";
...
export default function KosCriteriaSection() {
  ...
  const isFormValid = name.trim() !== "" && city.trim() !== "" && location.trim() !== "" && gender && recommendation && date;
  const handleSubmit = () => {
    if (containsBadWord(notes)) {
      alert("Catatan tambahan mengandung kata yang tidak diperbolehkan.");
      return;
    }
    ...
    const message = [
      ...
      `*Fasilitas:* ${facilities.length > 0 ? facilities.join(", ") : "Tidak ada preferensi khusus"}`,
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-[1fr_420px] gap-10">
        ...
        <div className="bg-white rounded-[2rem] p-10 shadow-sm border border-zinc-100">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {/* Nama */}
            <Input label="Nama Lengkap" placeholder="Masukkan nama lengkap Anda" value={name} onChange={setName} />
            {/* Jenis Kos */}
            <div>
              <label className="block mb-3 text-sm font-bold text-zinc-800">Jenis Kos</label>
              <div className="flex flex-wrap gap-3 text-zinc-800">
                {["Putra", "Putri", "Pasutri", "Campur"].map((item) => (
                  <button key={item} onClick={() => setGender(item)} className={cn("flex-1 flex items-center gap-2 px-5 py-2.5 rounded-xl border text-sm", gender === item ? "border-[#495C29] bg-[#EEF3E8]" : "border-zinc-200")}>
                    <span className={cn("w-4 h-4 rounded-full border flex items-center justify-center", gender === item ? "border-[#495C29]" : "border-zinc-300")}>
                      {gender === item && <span className="w-2 h-2 bg-[#495C29] rounded-full" />}
                    </span>
                    {item}
                  </button>
                ))}
            ...
            <Input label="Kabupaten / Kota Kos" placeholder="Pilih kabupaten / kota" value={city} onChange={setCity} />

            <Input label="Lokasi spesifik yang diharapkan" placeholder="Contoh: Dekat kampus UGM, Seturan" value={location} onChange={setLocation} />
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8 mt-12">
            <div>
              <label className="block text-sm font-bold mb-3 text-zinc-800">Budget per Bulan</label>
            ...
            <div>
              <label className="block text-sm font-bold mb-3 text-zinc-800">Jumlah Rekomendasi yang diinginkan</label>
              <div className="flex gap-4">
                {["3", "4"].map((num) => (
                  <button
                    key={num}
                    onClick={() => setRecommendation(num)}
                    className={cn("flex-1 flex items-center gap-2 px-5 py-2.5 rounded-xl border text-sm text-zinc-800", recommendation === num ? "border-[#495C29] bg-[#EEF3E8]" : "border-zinc-200")}
                  >
                    <span className={cn("w-4 h-4 rounded-full border flex items-center justify-center", recommendation === num ? "border-[#495C29]" : "border-zinc-300")}>
                      {recommendation === num && <span className="w-2 h-2 bg-[#495C29] rounded-full" />}
                    </span>
                    {num} Kos
                  </button>
                ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-12">
            <div>
              <label className="block text-sm font-bold mb-3 text-zinc-800">Rencana tanggal dihuni</label>
            ...
            <div>
              <label className="block text-sm font-bold mb-3 text-zinc-800">Catatan Tambahan (Opsional)</label>
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Catatan tambahan" className="w-full h-[52px] border border-zinc-200 rounded-xl px-4 py-3 outline-none resize-none text-zinc-800" />
            </div>
          </div>
          ...
          <button
            ...
            className={cn("mt-14 w-full py-4 rounded-2xl font-bold text-lg transition-all shadow-md", isFormValid ? "bg-[#495C29] hover:bg-[#3e4f23] text-white" : "bg-zinc-300 text-zinc-500 cursor-not-allowed")}
          >
            Kirim Kriteria Saya
          </button>
        </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-zinc-100 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <p className="font-bold text-[#495C29]">Informasi Penting</p>
            </div>
...
function Input({ label, placeholder, value, onChange }: { label: string; placeholder: string; value: string; onChange: (val: string) => void }) {
  return (
    <div>
      <label className="block text-sm font-bold mb-3 text-zinc-800">{label}</label>
      <input type="text" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full border border-zinc-200 rounded-xl px-4 py-3 outline-none placeholder:text-zinc-400 text-zinc-900" />
    </div>
  );
}
